...
int main(void) {
	...
	while (1) {
		printf("숫자를 입력하시오(9999를 누르면 종료됩니다):");
		scanf_s("% d", &temp);
		if (9999 == temp) break;
	...
	//==========2.모든 입력받은 숫자를 출력=========
	//입력된 숫자를 출력하기 위해서 노드를 탐색할 포인터에 시작 노드의 주소 값을 대입함
	p = p_head;
	while (NULL != p) {
		if (p != p_head) {
	...
	p_tail = p_head;//반복문을 나오면 p_head값은 NULL.p_tail값도 NULL값으로 변경
